...
pub fn ser_list_json(list: ContactList, path_file: String) {
    ...
    if res.is_err() {
        println!("ОШИБКА сохранения конфига");
    }
}
...
pub fn de_list_json(cfg_path: String) -> Result<ContactList, ()> {
    ...
    unsafe {
        let des: Result<ContactList, simd_json::Error> = simd_json::serde::from_str::<ContactList>(&mut res_str);
        if des.is_err() {
